...
namespace tcpdirect {
TcpSendEventHandler::TcpSendEventHandler(std::string thread_id, int socket,
                                         size_t message_size,
                                         bool do_validation) {
...
bool TcpSendEventHandler::HandleEvents(unsigned events) {
  if (events & EPOLLOUT) {
...
bool TcpSendEventHandler::HandleEPollOut() {
  ssize_t ret = send(socket_, &(tx_buf_.get()[tx_offset_]),
                     message_size_ - tx_offset_, MSG_DONTWAIT | MSG_ZEROCOPY);
...
bool TcpSendEventHandler::HandleEPollErr() {
  char control[128];
  struct msghdr msg = {
      .msg_control = control,
...
double TcpSendEventHandler::GetTxBytes() {
